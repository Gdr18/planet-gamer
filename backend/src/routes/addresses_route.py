...
def add_address():
	...
	if new_address.user_id != current_user.id and current_user.role != RoleType.ADMIN.value:
...
def handle_address(address_id):
	...
	if current_user.id != address.user_id and current_user.role != RoleType.ADMIN.value:
